...
void main() {
  late AssistanceProviderInterface provideInterface;
  late AssistanceService assistanceService;

  setUp(() async {
    provideInterface = MockAssistanceProviderInterface();
    assistanceService = AssistanceService().init(provideInterface);
    var json = File("${Directory.current.path}/test/resources/assists.json")
        .readAsStringSync();

    when(provideInterface.getAssists()).thenAnswer((_) async => Future.sync(
